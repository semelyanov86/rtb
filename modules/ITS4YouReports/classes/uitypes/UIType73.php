...
class UIType73 extends UITypes {
    protected $relModuleName = 'Accounts';
    protected $oth_as = '_73';
    ...
    public function getStJoinSQL(&$join_array, &$columns_array) {
        if (isset($this->params["fieldid"]) && $this->params["fieldid"] != "") {
            ...
            if (!array_key_exists(" vtiger_account AS vtiger_account" . $fieldid_alias . " ", $join_array)) {
                ...
                $join_array[" vtiger_account AS vtiger_account" . $fieldid_alias . " "]["using"] = $tablename . $old_oth_as . $old_oth_fieldid . $mif_as . "." . $columnname;
            }
            if (!array_key_exists(" vtiger_crmentity AS vtiger_crmentity".$this->oth_as . $fieldid_alias . " ", $join_array)) {
                ...
                $join_array[" vtiger_crmentity AS vtiger_crmentity".$this->oth_as . $fieldid_alias . " "]["using"] = "vtiger_account" . $fieldid_alias . ".accountid ";
            }
        }
    }
...
//            show("Join array uitype73 getJoinSQL",$this->params,$join_array);
        }
        if ($this->params["columnname"] == "accountid") {
...
//show("Columns array 73 getJoinSQL",$columns_array,"END JOIN<br />-------------");
    }
...
//show("<font color='green'>fielduitype73_IN_P_".$field_uitype,$join_array,"</font>");
    ...
    public function getSelectedFieldCol($selectedfields) {
        ...
        if ($selectedfields[1] == "accountid") {
            $return = "vtiger_account" . $fieldid_alias . "." . "accountname";
