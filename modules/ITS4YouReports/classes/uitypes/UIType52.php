...
class UIType52 extends UITypes {
    protected $id_cols_array = array("user_name", "modifiedby", "smcreatorid", "smownerid");
    protected $relModuleName = 'Users';
    protected $oth_as = '_52';
    ...
    public function getStJoinSQL(&$join_array, &$columns_array) {
        if (isset($this->params["fieldid"]) && $this->params["fieldid"] != "") {
            ...
            if (!array_key_exists(" vtiger_users AS vtiger_users" . $fieldid_alias . " ", $join_array)) {
                ...
                $join_array[" vtiger_users AS vtiger_users" . $fieldid_alias . " "]["using"] = $tablename . "." . $columnname;
            }
            if (!array_key_exists(" vtiger_groups AS vtiger_groups" . $fieldid_alias . " ", $join_array)) {
                ...
                $join_array[" vtiger_groups AS vtiger_groups" . $fieldid_alias . " "]["using"] = $tablename . "." . $columnname;
            }
        }
    }
    ...
    public function getJoinSQL(&$join_array, &$columns_array) {
        if (isset($this->params["fieldid"]) && $this->params["fieldid"] != "") {
            $fieldid_alias = $old_oth_fieldid = $old_oth_as = "";
            if ($this->params["fieldid"] != "") {
                $fieldid_alias = "_" . $this->params["fieldid"];
            }
            ...
            $join_tablename_alias = $this->params["join_tablename_alias"] = "vtiger_users" . $fieldid_alias;
            if (!array_key_exists(" vtiger_users AS vtiger_users" . $fieldid_alias . " ", $join_array) && !in_array($this->params["old_oth_fieldid"], array("inv"))) {
                $join_col = "vtiger_users" . $fieldid_alias . ".id";
                $join_array[" vtiger_users AS vtiger_users" . $fieldid_alias . " "]["joincol"] = $join_col;
                $userid_fld = $join_col;
                $join_array[" vtiger_users AS vtiger_users" . $fieldid_alias . " "]["using"] = $tablename . $old_oth_as . $old_oth_fieldid . $mif_as . "." . $columnname;
            }
            if (!array_key_exists(" vtiger_groups AS vtiger_groups" . $fieldid_alias . " ", $join_array) && !in_array($this->params["old_oth_fieldid"], array("inv"))) {
                $join_array[" vtiger_groups AS vtiger_groups" . $fieldid_alias . " "]["joincol"] = "vtiger_groups" . $fieldid_alias . ".groupid";
                $join_array[" vtiger_groups AS vtiger_groups" . $fieldid_alias . " "]["using"] = $tablename . $old_oth_as . $old_oth_fieldid . $mif_as . "." . $columnname;
            }
        }
...
//show("<font color='green'>fielduitypeJOJO52",$join_array,$this->params["field_uitype"],$columns_array,"</font>");
    }
...
//show("<font color='green'>fielduitype52_IN_P_".$field_uitype,$related_join_array,$params["using_array"],"</font>");
