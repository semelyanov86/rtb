...
class UIType59 extends UITypes {
    protected $relModuleName = 'Products';
    protected $oth_as = '_59';
    ...
    public function getStJoinSQL(&$join_array, &$columns_array) {
        if (isset($this->params["fieldid"]) && $this->params["fieldid"] != "") {
            ...
            if (!array_key_exists(" vtiger_products AS vtiger_products" . $fieldid_alias . " ", $join_array)) {
                $join_array[" vtiger_products AS vtiger_products" . $fieldid_alias . " "]["joincol"] = "vtiger_products" . $fieldid_alias . ".productid";
                $join_array[" vtiger_products AS vtiger_products" . $fieldid_alias . " "]["using"] = $tablename . $old_oth_fieldid . $mif_as . "." . $columnname;
            }
            if (!array_key_exists(" vtiger_crmentity AS vtiger_crmentity".$this->oth_as . $fieldid_alias . " ", $join_array)) {
                ...
                $join_array[" vtiger_crmentity AS vtiger_crmentity".$this->oth_as . $fieldid_alias . " "]["using"] = "vtiger_products" . $fieldid_alias . ".productid ";
            }
        }
    }
    ...
    public function getJoinSQL(&$join_array, &$columns_array) {
        ...
        if (in_array($this->params["columnname"],array("productid","product_id"))) {
            $this->params["join_tablename_alias"] = "vtiger_products" . $fieldid_alias;
...
//show("<font color='green'>fielduitype59_IN_P_".$field_uitype,$related_join_array,$params["using_array"],"</font>");
    ...
    public function getSelectedFieldCol($selectedfields) {
        ...
        if ($selectedfields[1] == "productid") {
            $return = "vtiger_products" . $fieldid_alias . "." . $selectedfields[1];
