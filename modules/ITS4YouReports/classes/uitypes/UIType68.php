...
class UIType68 extends UITypes {
    protected $oth_as = '_68';
    ...
    public function getStJoinSQL(&$join_array, &$columns_array) {
        if (isset($this->params["fieldid"]) && $this->params["fieldid"] != "") {
            ...
            if (!array_key_exists(" vtiger_account AS vtiger_account" . $fieldid_alias . " ", $join_array)) {
                $join_array[" vtiger_account AS vtiger_account" . $fieldid_alias . " "]["joincol"] = "vtiger_account" . $fieldid_alias . ".accountid";
                $join_array[" vtiger_account AS vtiger_account" . $fieldid_alias . " "]["using"] = $tablename . $old_oth_fieldid . $mif_as . "." . $columnname;
            }
            if (!array_key_exists(" vtiger_crmentity AS vtiger_crmentity".$this->oth_as."a" . $fieldid_alias . " ", $join_array)) {
                $join_array[" vtiger_crmentity AS vtiger_crmentity".$this->oth_as."a" . $fieldid_alias . " "]["joincol"] = "vtiger_crmentity".$this->oth_as."a" . $fieldid_alias . ".crmid";
                $join_array[" vtiger_crmentity AS vtiger_crmentity".$this->oth_as."a" . $fieldid_alias . " "]["using"] = "vtiger_account" . $fieldid_alias . ".accountid ";
            }
            ...
            if (!array_key_exists(" vtiger_contactdetails AS vtiger_contactdetails" . $fieldid_alias . " ", $join_array)) {
                ...
                $join_array[" vtiger_contactdetails AS vtiger_contactdetails" . $fieldid_alias . " "]["using"] = $tablename . $old_oth_fieldid . $mif_as . "." . $columnname;
            }
            if (!array_key_exists(" vtiger_crmentity AS vtiger_crmentity".$this->oth_as."c" . $fieldid_alias . " ", $join_array)) {
                $join_array[" vtiger_crmentity AS vtiger_crmentity".$this->oth_as."c" . $fieldid_alias . " "]["joincol"] = "vtiger_crmentity".$this->oth_as."c" . $fieldid_alias . ".crmid";
                $join_array[" vtiger_crmentity AS vtiger_crmentity".$this->oth_as."c" . $fieldid_alias . " "]["using"] = "vtiger_contactdetails" . $fieldid_alias . ".contactid ";
            }
        }
    }
    ...
    public function getJoinSQLbyFieldRelation(&$join_array, &$columns_array) {
        $module = vtlib_getModuleNameById($this->params["formodule"]);
        $related_focus = CRMEntity::getInstance($module);
        ...
        if ($uirel_row["tablename"] == "vtiger_crmentity") {
            $m_prefix = substr(strtolower($module), 0, 1);
            $oth_as = $this->oth_as . $m_prefix;
...
//show("<font color='green'>fielduitype68_IN_P_".$field_uitype,$join_array,"</font>");
    ...
    public function getSelectedFieldCol($selectedfields) {
        ...
        if ($selectedfields[1] == "accountid") {
            $return = "vtiger_account" . $fieldid_alias . "." . $selectedfields[1];
        } elseif ($selectedfields[1] == "contactid") {
            $return = "vtiger_contactdetails" . $fieldid_alias . "." . $selectedfields[1];
            // dokoncit ifnutie 68
        } elseif ($selectedfields[0] == "vtiger_crmentity") {
            $return = $selectedfields[0] . $this->oth_as."a" . $fieldid_alias . "." . $selectedfields[1];
        } elseif ($selectedfields[0] == "vtiger_crmentity") {
            $return = $selectedfields[0] . $this->oth_as."c" . $fieldid_alias . "." . $selectedfields[1];
        } else {
