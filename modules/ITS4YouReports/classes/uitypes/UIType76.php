...
class UIType76 extends UITypes {
    protected $id_cols_array = array("potentialid", "potentialname");
    protected $relModuleName = 'Potentials';
    protected $oth_as = '_76';
    ...
    public function getStJoinSQL(&$join_array, &$columns_array) {
        if (isset($this->params["fieldid"]) && $this->params["fieldid"] != "") {
            ...
            if ($this->params["old_oth_fieldid"] != "" && $this->params["report_primary_table"]!=$tablename) {
            ...
            if (!array_key_exists(" vtiger_potential AS vtiger_potential" . $fieldid_alias . " ", $join_array)) {
                $join_array[" vtiger_potential AS vtiger_potential" . $fieldid_alias . " "]["joincol"] = "vtiger_potential" . $fieldid_alias . ".potentialid";
                $join_array[" vtiger_potential AS vtiger_potential" . $fieldid_alias . " "]["using"] = $tablename . $old_oth_as . $old_oth_fieldid . $mif_as . "." . $columnname;
            }
            if (!array_key_exists(" vtiger_crmentity AS vtiger_crmentity".$this->oth_as . $fieldid_alias . " ", $join_array)) {
                ...
                $join_array[" vtiger_crmentity AS vtiger_crmentity".$this->oth_as . $fieldid_alias . " "]["using"] = "vtiger_potential" . $fieldid_alias . ".potentialid ";
            }
        }
    }
    ...
    public function getJoinSQL(&$join_array, &$columns_array) {
        ...
        $fieldid_alias = $oth_as = $mif_as = $old_oth_fieldid = "";
        if ($this->params["fieldid"] != "") {
            $fieldid_alias = "_" . $this->params["fieldid"];
        }
        ...
        if ($this->params["primary_table_name"]!=$join_tablename_alias && !array_key_exists($join_alias, $join_array) && !in_array($this->params["old_oth_fieldid"], array("inv")) && $this->params["tablename"] != $related_focus->table_name) {
            if ($this->params["tablename"] != $this->params["report_primary_table"] && $this->params["tablename"] != $this->params["primary_table_name"]) {
                if (is_array($this->params["table_index"])) {
                    $join_table_index = $this->params["table_index"][$this->params["tablename"]];
                } elseif ($this->params["table_index"] != "") {
                    $join_table_index = $this->params["table_index"];
                } else {
                    $join_table_index = $this->params["fieldname"];
                }
                $join_array[$join_alias]["joincol"] = $join_tablename_alias . "." . $join_table_index;
                ...
                }elseif(isset($this->params["using_array"]) && !empty($this->params["using_array"]) && $this->params["using_array"]['join']['tablename']!='' && $this->params["using_array"]['join']['columnname']!=''){
                    $join_array[$join_alias]["using"] = $this->params["using_array"]['join']['tablename'] . "." . $this->params["using_array"]['join']['columnname'];
                }
            }
        }
        // ITS4YOU-CR SlOl 3. 12. 2015 10:08:38  - fix dev. record=108
        $this->params["old_oth_fieldid"] = $this->params["fieldid"];
        // ITS4YOU-END

        if (in_array($this->params["columnname"], $this->id_cols_array)) {
            $this->params["join_tablename_alias"] = "vtiger_potential" . $fieldid_alias;
...
// show("<font color='green'>fielduitype76_IN_P_".$field_uitype,$params,"</font>");
    ...
    public function getSelectedFieldCol($selectedfields) {
        ...
        if (in_array($selectedfields[1], $this->id_cols_array)) {
            $return = "vtiger_potential" . $fieldid_alias . "." . $selectedfields[1];
