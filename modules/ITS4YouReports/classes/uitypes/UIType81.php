...
class UIType81 extends UITypes {
    protected $relModuleName = 'Vendors';
    protected $oth_as = '_81';
    ...
    public function getStJoinSQL(&$join_array, &$columns_array) {
        if (isset($this->params["fieldid"]) && $this->params["fieldid"] != "") {
            ...
            if (!array_key_exists(" vtiger_vendor AS vtiger_vendor" . $fieldid_alias . " ", $join_array)) {
                $join_array[" vtiger_vendor AS vtiger_vendor" . $fieldid_alias . " "]["joincol"] = "vtiger_vendor" . $fieldid_alias . ".vendorid";
                $join_array[" vtiger_vendor AS vtiger_vendor" . $fieldid_alias . " "]["using"] = $tablename . $old_oth_as . $old_oth_fieldid . $mif_as . "." . $columnname;
            }
            if (!array_key_exists(" vtiger_crmentity AS vtiger_crmentity".$this->oth_as . $fieldid_alias . " ", $join_array)) {
                ...
                $join_array[" vtiger_crmentity AS vtiger_crmentity".$this->oth_as . $fieldid_alias . " "]["using"] = "vtiger_vendor" . $fieldid_alias . ".vendorid ";
            }
        }
    }
    ...
    public function getJoinSQL(&$join_array, &$columns_array) {
        ...
        if ($this->params["columnname"] == "vendorid") {
            $this->params["join_tablename_alias"] = "vtiger_vendor" . $fieldid_alias;
            ...
            $fld_alias = $join_tablename_alias . "." . $this->params["columnname"] . $fieldid_alias;
            $fld_hrefid = "";
        }
...
//show("<font color='green'>fielduitype81_IN_P_".$field_uitype,$related_join_array,$params["using_array"],"</font>");
    ...
    public function getSelectedFieldCol($selectedfields) {
        ...
        if ($selectedfields[1] == 'vendorid') {
            $return = "vtiger_vendor" . $fieldid_alias . "." . $selectedfields[1];
