...
Vtiger_List_Js("ITS4YouReports_List_Js",{
	...
	addReport : function(url){
		var listInstance = ITS4YouReports_List_Js.getInstance();
    ...
    savePinToDashboard : function(element, customParams) {
        ...
        app.request.post({data: params}).then(function (error,data) {
                if (data.duplicate) {
                        var params = {
                                message: app.vtranslate('JS_CHART_ALREADY_PINNED_TO_DASHBOARD', 'ITS4YouReports')
                        };
                        app.helper.showSuccessNotification(params);
                        listInstance.massActionPostOperations(data);
                } else {
                        var message = app.vtranslate('JS_CHART_PINNED_TO_DASHBOARD', 'ITS4YouReports');
	...
	registerEventForPinChartToDashboard: function () {
        ...
        this.getListViewContainer().on('click','.pinToDashboard',function (e) {
			...
			if(pinned) {
				...
				app.request.post({data: params}).then(function (error,data) {
					if(data.unpinned) {
						var message = app.vtranslate('JS_CHART_REMOVED_FROM_DASHBOARD', 'ITS4YouReports');
						app.helper.showSuccessNotification({message:message});
