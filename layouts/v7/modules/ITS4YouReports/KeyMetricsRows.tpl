...
jQuery().ready(function() {
    var ITS4YouKeyMetrics_Js = new ITS4YouReports_KeyMetricsList_Js();
        ...
        <div class="tab-content themeTableColor overflowVisible">
            <div class="tab-pane active" id="allValuesLayout">
                ...
                {if $CURRENT_USER->isAdminUser() || $KM_SMCREATOR==$currentuser_id}
                            <button class="btn btn-default" id="addNewKeyMetricsRow" onclick="window.location.href='index.php?module={$MODULE}&view=EditKeyMetricsRow&km_id={$KM_ID}&id=&reportid='">{vtranslate('LBL_ADD_NEW_KEY_METRICS_ROW',$MODULE)}</button>
                {/if}
                    </div>
            </div>
        </div>
