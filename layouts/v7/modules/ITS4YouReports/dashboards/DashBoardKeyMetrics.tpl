...
{strip}
    <div class="" style="position: relative; overflow: hidden; width: auto; height: 250px;">
        <div class="dashboardWidgetContent" style="padding: 5px; overflow: hidden; width: auto; height: 250px;">

            {if !empty($DATA)}
                {foreach item=keyMetricsRow from=$DATA}
                ...
                {/foreach}
            {else}
                {vtranslate('LBL_NO_DATA_TO_DISPLAY',$MODULE)}
            {/if}
        </div>
    </div>
