				...
				<table id="listview-table"  class="table {if $LISTVIEW_ENTRIES_COUNT eq '0'}listview-table-norecords {/if} listview-table">
							...
							<tr class="searchRow">
								...
								{foreach item=LISTVIEW_HEADER key=LISTVIEW_HEADER_KEY from=$LISTVIEW_HEADERS}
									<th>
										...
										{elseif $DATA_TYPE == 'picklist' || $DATA_TYPE == 'user'}
											{assign var=PICKLIST_VALUES value=$LISTVIEW_HEADER.picklistValues}
					...
					<tbody class="overflow-y">
						{foreach item=LISTVIEW_ENTRY from=$LISTVIEW_ENTRIES name=listview}
							<tr class="listViewEntries" data-id='{$LISTVIEW_ENTRY->get("reportid")}' data-recordUrl='{$LISTVIEW_ENTRY->getDetailViewUrl()}' id="{$MODULE}_listView_row_{$smarty.foreach.listview.index+1}">
									...
									<td class="listViewEntryValue" data-name="{$LISTVIEW_HEADERNAME}" title="{$LISTVIEW_ENTRY_RAWVALUE}" data-rawvalue="{$LISTVIEW_ENTRY_RAWVALUE}" data-field-type="">
										<span class="fieldValue">
											<span class="value textOverflowEllipsis">
												...
												{elseif $LISTVIEW_HEADERNAME eq 'tablabel'}
