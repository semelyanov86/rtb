    ...
    jQuery(document).ready(function() {
        ...
        if (selObj) {
            ...
            if (currField.value != null && currField.value.length != 0) {
                ...
                if (std_filter_columns_arr.indexOf(selected_value) > -1 || fieldtype=='D') {
...
function addSortColumnRow(){
    ...
    sortColumnBase = replaceAll('row-fluid', 'chzn-select row-fluid', sortColumnBase);
...
//return false;
            if(jQuery.inArray(fop_type_val,JSON.parse(jQuery('#fld_date_options').val()))>-1){
                ...
                if(fop_type_val=="custom"){
                    ...
                    app.registerEventForDateFields('#jscal_field_sdate_val_' + row_i);
                    app.registerEventForDateFields('#jscal_field_edate_val_' + row_i);
                }
            }
        }
    );
...
function setRelPopupClick(clickBtn,columnIndex){
    ...
    clickBtnObj.on('click',function(e) {
        ...
        jQuery('#fc_fval_' + columnIndex).removeClass('chzn-done');
        app.changeSelectElementView(jQuery('#fc_fval_' + columnIndex));
    })
}
...
{
    jQuery('#' + el + ' option:selected').removeAttr("selected").trigger('liszt:updated');
}
...
function emptyCheck4You(fldName, fldLabel, fldType) {
    ...
    if (fldType == "text") {
        if (currObj.value=="" || currObj.value.replace(/^\s+/g, '').replace(/\s+$/g, '').length == 0) {
            ...
            alert(fldLabel + app.vtranslate('VAL_CANNOT_BE_EMPTY',moduleName));
            try {
                currObj.focus()
            } catch (error) {
...
function ITS4YouReportsDateValidate(fldName, fldLabel, type) {
...
function generatePDF(id, pdfmaker_active, is_test_write_able, div_id) {
    if (pdfmaker_active == true || pdfmaker_active == 1) {
        ...
        if (report_filename_obj && report_filename_obj.value != "") {
            ...
            /*
            if (document.getElementById("reports4you_widget_"+id) != null && typeof document.getElementById("reports4you_widget_"+id) != 'undefined' && document.getElementById("reports4you_widget_"+id) != "") {
                saveChartAsBinary();
            }else{
                window.open(report_filename_obj.value, "width=800,height=650,resizable=1,scrollbars=1,left=100");
            }
            */
            return true;
...
function printDiv(){
    ...
    document.getElementById('break_row').style.display = "none";
    document.getElementById('chart_div').style.display = "none";
    var reports4you_html = document.getElementById('reports4you_html').innerHTML;
    mywindow.document.write(reports4you_html);
...
function exportToPDF(report_filename){
    ...
    var reports4you_name = jQuery('.rpt4youGrpHeadInfoText').html();
    ...
    let topAsDiv = 'top_'
    if ('undefined' === typeof jQuery('#reports4you_widget_' + topAsDiv + id).html() || null === jQuery('#reports4you_widget_' + topAsDiv + id).html()) {
        topAsDiv = '';
    }
...
function addNewCustomCalculation(){
    ...
    jQuery('#cc_options_'+cc_count).removeClass("chzn-done");

    app.changeSelectElementView(jQuery('#cc_options_'+cc_count));
